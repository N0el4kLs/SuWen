...
// Init 初始化IP纯真库
func Init() {
    if !util.Exists(qqwryFile) {
        logging.Logger.Infof("%s not find, download...", qqwryFile)
        err := Download(qqwryFile)
        if err != nil {
            logging.Logger.Errorf("download %s error:%v", qqwryFile, err)
            return
        }
    }
    
    fs, err := os.OpenFile(path.Join("./", qqwryFile), os.O_RDONLY, 0777)
    if err != nil {
        logging.Logger.Errorln("qqwry open err:" + err.Error())
        return
    }
    if DB, err = NewQQwryFS(fs); err != nil {
        logging.Logger.Errorln("qqwry init err:" + err.Error())
        return
    }
}

// NewQQwryFS new database from path
func NewQQwryFS(fs *os.File) (qqwry *QQwry, err error) {
    ...
    fileData, err = io.ReadAll(fileInfo.FileBase)
    if err != nil {
        return nil, err
    }
