...
func (a *AVDCrawler) parsePage(ctx context.Context, page int) ([]*VulnInfo, error) {
    ...
    count := sel.Length()
    if count == 0 {
        logging.Logger.Errorf("invalid response is \n%s", resp.Dump())
    ...
    for _, href := range hrefs {
        ...
        if err != nil {
            logging.Logger.Errorf("%s %s", err, vulnLink)
            return results, nil
        }
...
func (a *AVDCrawler) parseSingle(ctx context.Context, vulnLink string) (*VulnInfo, error) {
    logging.Logger.Debugf("parsing vuln %s", vulnLink)
    ...
    if !cveIDRegexp.MatchString(cveID) {
        cveID = ""
        logging.Logger.Debugf("cve id not found in %s", vulnLink)
    }
