...
func (t *OSCSCrawler) GetUpdate(ctx context.Context, pageLimit int) ([]*VulnInfo, error) {
    ...
    for i := 1; i <= pageCount; i++ {
        ...
        logging.Logger.Infof("got %d vulns from page %d", len(pageResult), i)
    ...
    _, err := t.client.R().
        ...
        AddRetryCondition(func(resp *req.Response, err error) bool {
            ...
            if err = resp.UnmarshalJson(&body); err != nil {
                logging.Logger.Warnf("unmarshal json error, %s", err)
                return true
            }
            if body.Code != 200 || !body.Success {
                logging.Logger.Warnf("failed to get page count, msg: %s, retrying", body.Info)
                return true
            }
            if body.Data.Total <= 0 {
                logging.Logger.Warnf("invalid total size %d, retrying", body.Data.Total)
                return true
            }
            return false
        }).
    ...
    for _, d := range body.Data.Data {
        ...
        if err != nil {
            if !errors.Is(err, context.Canceled) {
                logging.Logger.Errorf("failed to parse %s, %s", d.Url, err)
            }
            continue
        }
        info.Tags = tags
